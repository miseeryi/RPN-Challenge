    ...
    for(let i=0; i<arrSort.length; i++){
        for(let j=i +1; j<arrSort.length; j++){
            if (setArr.length === arr.length){
                ...
                if(arrSort[i] === arrSort[j] && arrSort[i+1] === arrSort[j+1] && arrSort[i+2] === arrSort[j+2]){
                    return -1
                }
                else if(arrSort[i] === arrSort[j]){
                    return arrSort[i]
                } 
                else if(setArr.length === arrSort.length) return -1
            }
        }
    }
}
